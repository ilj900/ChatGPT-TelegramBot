...
# Helper function to calculate total cost of tokens and images used
...
# If user's balance is greater than his current usage, then he is considered active
...
def account_payment(user_name: str, paid, database_name: str):
    ...
    result = cursor.fetchone()
    if result:
        # If there is already an entry with user in database, then just update
...
# Update how much i/o tokens and images were used/generated by the user. Incrementally
def update_token_count(user_name: str, input_token_count, output_token_count, images_generated, database_name: str):
    ...
    if result:
        ...
        cursor.execute('''
                    INSERT INTO statistics (user_name, input_token_count, output_token_count, images_generated, current_balance)
                    VALUES (?, ?, ?, ?, ?)
                ''', (user_name, input_token_count, output_token_count, images_generated, 0))

    connection.commit()
    connection.close()


# Function to split a long text into a blocks of text and code
